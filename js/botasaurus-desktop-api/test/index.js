...
describe('Excel functions', () => {
  it('should write and read Excel file', () => {
    const data = [
      { name: 'John', age: 30 },
      { name: 'Alice', age: 25 },
    ];
    const filename = writeExcel(data, 'test.xlsx', false);
    const readData = readExcel(filename);
    assert.deepStrictEqual(readData, data);
  });

  it('should write and read temporary Excel file', () => {
    const data = [
      { name: 'Bob', age: 35 },
      { name: 'Eve', age: 28 },
    ];
    writeTempExcel(data, false);
    const readData = readTempExcel();
    assert.deepStrictEqual(readData, data);
  });
});